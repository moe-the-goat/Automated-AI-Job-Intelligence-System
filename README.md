...
After every pipeline run, [pipeline/core_feedback_page.py](pipeline/core_feedback_page.py) writes a static HTML page to `docs/feedback_global.html` (and `docs/feedback_local.html` for the local pipeline). The page lists every job from that run with a small dropdown per row (`Applied`, `Bookmarked`, `Not relevant`, `Block company`, `Wrong location`, `Other`) and an optional note field. A single "Submit All" button at the bottom POSTs the collected entries to a Cloudflare Worker. The Worker holds a fine-grained GitHub PAT in its own environment variables and writes `data/feedback_pending.json` into the private logs repo on the page's behalf. Source for the Worker lives in [feedback_worker/worker.js](feedback_worker/worker.js), with setup steps in [feedback_worker/README.md](feedback_worker/README.md). No GitHub token is ever embedded in the HTML page; that matters because GitHub's secret-scanning revokes any PAT it finds in a public commit, which would break a direct-API design on the first push.
...
The Worker enforces a strict `Origin` check so only the GitHub Pages domain can submit, and caps a single submission at 100 entries. The PAT it holds is scoped to write a single file path in a single private repo. The architecture means the public HTML page contains nothing more sensitive than the Worker's public URL.
...
|   |-- core_local_sources.py         Palestinian local sources (ATS + DDG + Telegram + jobs.ps)
...
|   |-- core_feedback.py              feedback constants + RAG threshold + digest prompt
|   |-- core_feedback_supabase.py     Supabase-backed feedback corpus + embeddings for RAG
...
|   |-- core_websearch.py             Google Programmable Search + DDG fallback
...
> The companion **web app** (signup, per-user dashboard, feedback, and the
> approval-based access gate) lives in a separate Next.js repo deployed on
> Vercel. It replaced the old static GitHub-Pages feedback pages and the
> Cloudflare Worker that proxied their writes.
